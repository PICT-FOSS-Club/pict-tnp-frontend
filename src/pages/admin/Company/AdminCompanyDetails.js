...
const AdminCompanyDetails = () => {
    ...
    const handleDeleteJob = async (e) => {
        ...
        if (confirm === "CONFIRM") {
            ...
            alert("Unable to process the delete operation!");
        }
    }
    let datestring = company.endDate.split('T');
    ...
    return (
        <div className="container company bootstrap snippets bootdey">
                        ...
                        <div className="panel-body bio-graph-info">
                            ...
                            <div>
                                <label htmlFor="12per" className="form-label">
                                    Update registration end date and time :   {datestring[0] + " & " + datestring[1].split('.')[0]}
                                </label>
                                <div className="input-group has-validation" style={{ width: "300px" }}>
                                    <input
                                        type="datetime-local"
                                        ...
                                        required
                                    /> <button type="button" className="mx-2 btn btn-primary">Update</button>
                                </div>
                            </div>
                        </div>
                        <div className="panel-body bio-graph-info">
                                    ...
                                    <div className="alert my-2 roundDetails alert-secondary" title={'Date: ' + round.date + ' Time: ' + round.time + ' Venue: ' + round.venue} role="alert" key={key}>
                                        <div>
                                            ...
                                            <div className="options">
                                                ...
                                                <button id={key + 1} type="button" className="btn btn-dark" data-bs-toggle="modal" data-bs-target="#UpdateCompanyForm" data-tooltip="Edit round" value="editround" onClick={() => updateRoundState(round.roundNo)}><i className="bi bi-pencil-square"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                ))}
            ...
            <button type="button" style={{ float: "right" }} onClick={handleDeleteJob} className="m-3 btn btn-outline-danger">Delete</button>
